...
public class CamarillaPivotIndicator implements IIndicator, IDrawingIndicator {
    ...
    public void onStart(IIndicatorContext context) {
        indicatorInfo = new IndicatorInfo("CAMPIVOT", "Camarilla Pivots", "Overlap Studies", true, false, true, 2, 1, 11);
        indicatorInfo.setSparseIndicator(true);
        indicatorInfo.setRecalculateAll(true);
//        indicatorInfo.setSupportedDataTypes(DataType.TICKS, DataType.TIME_PERIOD_AGGREGATION);
        ...
        }, new OutputParameterInfo("Resistance (R4)", OutputParameterInfo.Type.DOUBLE, OutputParameterInfo.DrawingStyle.LEVEL_LINE, false) {
            {
                setDrawnByIndicator(true);
            }
        }, new OutputParameterInfo("Support (S4)", OutputParameterInfo.Type.DOUBLE, OutputParameterInfo.DrawingStyle.LEVEL_LINE, false) {
            {
                setDrawnByIndicator(true);
            }
        }, new OutputParameterInfo("Resistance (R5)", OutputParameterInfo.Type.DOUBLE, OutputParameterInfo.DrawingStyle.LEVEL_LINE, false) {
            {
                setDrawnByIndicator(true);
            }
        }, new OutputParameterInfo("Support (S5)", OutputParameterInfo.Type.DOUBLE, OutputParameterInfo.DrawingStyle.LEVEL_LINE, false) {
            {
                setDrawnByIndicator(true);
            }
        }};
    ...
    public IndicatorResult calculate(int startIndex, int endIndex) {
        ...
        for (int i = 0; i < inputs[1].length; i++){
            ...
            if (prevBar != null && timeIndex > -1 &&
                ...
                outputs[0][timeIndex - startIndex] = (close + prevBar.getHigh() + prevBar.getLow())/3; //P
                outputs[1][timeIndex - startIndex] = close + (diff) * 1.1 / 12; //R1
                outputs[2][timeIndex - startIndex] = close - (diff) * 1.1 / 12; //S1
                outputs[3][timeIndex - startIndex] = close + (diff) * 1.1 / 6;  //R2
                outputs[4][timeIndex - startIndex] = close - (diff) * 1.1 / 6;  //S2
                outputs[5][timeIndex - startIndex] = close + (diff) * 1.1 / 4;  //R3
                outputs[6][timeIndex - startIndex] = close - (diff) * 1.1 / 4; //S3
                outputs[7][timeIndex - startIndex] = close + diff * 1.1/2; //R4
                outputs[8][timeIndex - startIndex] = close - diff * 1.1/2 ; //S4
                double r5 = (prevBar.getHigh() / prevBar.getLow()) * close; //R5
                outputs[9][timeIndex - startIndex] = r5;
                outputs[10][timeIndex - startIndex] = close - (r5 - close); //S5
            }
            if (timeIndex > -1) {
                prevBar = inputs[1][i];
            }
