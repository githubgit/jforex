    ...
    public IndicatorResult calculate(int startIndex, int endIndex) {
        ...
        for (int i = 0; i < inputs[1].length; i++){
            ...
            if (prevBar != null && timeIndex > -1 &&
                ...
                // R1
                outputs[1][timeIndex - startIndex] = 2 * p -  prevBar.getLow();
                // S1
                outputs[2][timeIndex - startIndex] = 2 * p -  prevBar.getHigh();
                // R2
                outputs[3][timeIndex - startIndex] = p +  prevBar.getHigh() -  prevBar.getLow();
                // S2
                outputs[4][timeIndex - startIndex] = p -  (prevBar.getHigh() - prevBar.getLow());
                // R3
                outputs[5][timeIndex - startIndex] = prevBar.getHigh()  + 2 * (p - prevBar.getLow());
                // S3
                outputs[6] [timeIndex - startIndex] = prevBar.getLow() - 2 * (prevBar.getHigh() - p);
            }
            if (timeIndex > -1) {
                prevBar = inputs[1][i];
            }
